...
public class OutputLayer extends AbstractNeuronNetwork
{
	...
	{
		config.setLayerSize(parentConfig.outputLayerSize).setL2(parentConfig.L2)
				.setLossType(parentConfig.lossType).setNormalized(parentConfig.normalized)
				.setRandomFunction(parentConfig.randomFunction)
				.setUseRegularization(parentConfig.useRegularization)
				.setUseAdaGrad(parentConfig.useAdaGrad).setLearnRate(parentConfig.learnRate);
